...
def test_cli_web_command_missing_streamlit(tmp_dir):
    """llmreplay web <run_id> prints install hint when streamlit is absent."""
    from llmreplay.core.context import record as _record

    with _record("web_test_run", base_dir=tmp_dir, seed=1):
        pass

    runner = CliRunner()
    # Patch web.launch to avoid actually launching streamlit
    with patch("llmreplay.web.launch") as mock_launch:
        result = runner.invoke(cli, ["web", "web_test_run", "--dir", str(tmp_dir)])
    assert result.exit_code == 0
