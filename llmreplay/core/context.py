...
def record(
    ...
    *,
    ...
    Works with async agent code too — just use ``with``, not ``async with``::

        with record("my_run") as session:
            result = await my_agent.run(query)

    The session is set on a ContextVar so hooks installed during this block
    can find it from any thread or asyncio task spawned within the context.
    """
    ...
    # Persist seed + metadata as step-0 and step-1 events
    session.emit(EventKind.RANDOM_SEED, {"seed": actual_seed})
    session.emit(EventKind.METADATA, {"metadata": session.metadata})

    # Seed Python's built-in RNG for reproducibility
    random.seed(actual_seed)

    # Seed numpy if present — optional dependency, silently skipped if absent
    try:
